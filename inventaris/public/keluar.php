...
if ($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    if (isset($_POST['add'])){
        if (!empty($_POST['barang']) && !empty($_POST['jumlah'])){
            $check_stok = $conn->query("SELECT stok - {$_POST['jumlah']} FROM stock WHERE id = {$_POST['barang']}")->fetch_array()[0];
            if ($check_stok > 0){
                $jumlah = $_POST['jumlah'];
    ...
    if (isset($_POST['keluarkan'])){
        ...
        if ($lenght <= 0 ){
            ...
            $insert_pencatatan = $conn->query("INSERT INTO pencatatan_barang (users_id, tanggal, jenis_catatan) VALUES ({$_SESSION['user_id']}, NOW(), 'keluar')");
        ...
        <div class="container-fluid my-5 ">
            <div class="row d-flex bg-secondary-2 rounded">
                ...
                <div class="d-flex justify-content-between align-items-center mt-3">
                    <h3 class="text-white">Barang Keluar</h3>
                    <!-- <a href=""></a> -->
                </div>
                ...
                <form action="" method="post">
                    <button class="btn bg-light-2 mb-2" type="submit" name="keluarkan">Keluarkan</button>
                </form>
            </div>
        </div>
    </div>
